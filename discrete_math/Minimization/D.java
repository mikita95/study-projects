...
public class D {
    ...
    private final static String FILE_NAME = "fastminimization";
    ...
    private static class DFA {
        ...
        public void slowMinimize() {
            ...
            ArrayList<HashSet<Integer>> P = temp();
        ...
        public ArrayList<HashSet<Integer>> partition() {
            ...
            for (int c = 0; c < power; c++) {
                if (F.size() < QmF.size())
                    queue.add(new Pair<>(F, c));
                else
                    queue.add(new Pair<>(QmF, c));
            }
            ArrayList<Integer> Count = new ArrayList<>();
            ArrayList<Integer> Twin = new ArrayList<>();
            for (int i = 0; i < 2 * statesNumber; i++) {
                Count.add(0);
                Twin.add(-1);
            }
            while (!queue.isEmpty()) {
                ...
                for (Integer q : pair.getKey()) {
                    Inv.get(q).get(pair.getValue()).stream().filter(r -> r != -1).forEach(r -> {
                        int i = Class.get(r);
                        if (Count.get(i) == 0)
                            Involved.add(i);
                ...
                Involved.stream().filter(i -> Count.get(i) < P.get(i).size()).forEach(i -> {
                    P.add(new HashSet<>());
                    Twin.set(i, P.size() - 1);
                });
                int[] p = new int[pair.getKey().size()];
                int t = 0;
                for (Integer q : pair.getKey()) {
                    p[t] = q;
                    t++;
        ...
        public void minimize() {
            ...
            HashSet<Integer> newAccept = new HashSet<>();
            if (Class.get(start) != 0 && P.size() > 1) {
                int change = Class.get(start);
                for (int i = 0; i < Class.size(); i++) {
                    if (Class.get(i) == change)
                        Class.set(i, 0);
                    else if (Class.get(i) == 0)
                        Class.set(i, change);
                }
            }
            int st = Class.get(statesNumber - 1);
            for (Integer i : accept) {
                int cl = Class.get(i);
                if (cl < st)
                    newAccept.add(Class.get(i));
                else if (cl > st) newAccept.add(Class.get(i) - 1);
            }
            ...
            for (int i = 0; i < statesNumber; i++) {
                for (int c = 0; c < power; c++) {
                    ...
                    int cl = Class.get(i);
                    // if (cl != st && r != statesNumber - 1) {
                    if (cl < st) {
                        if (Class.get(r) < st)
                            newTable.get(cl).set(c, Class.get(r));
                        else if (Class.get(r) > st) newTable.get(cl).set(c, Class.get(r) - 1);
                    } else if (cl > st) {
                        if (Class.get(r) < st)
                            newTable.get(cl - 1).set(c, Class.get(r));
                        else if (Class.get(r) > st) newTable.get(cl - 1).set(c, Class.get(r) - 1);
                    }
                    // }
                }
            }
        ...
        public ArrayList<HashSet<Integer>> temp() {
            int n = statesNumber;
            int charsCount = power;
            ArrayList<ArrayList<ArrayList<Integer>>> from = getInvertedTable();
...

            {
                ...
                classCount += 2;
                for (int i = 0; i < n; i++) {
                    int classId = isAcceptable(i) ? finishClassId : notFinishClassId;
                    classSize[classId]++;
            ...
            while (nextQ <= lastQ) {
                ...
                fromSListLast = -1;
                for (Node node : classHead[curClass]) {
                    int state = node.value;
                    for (int fromS : from.get(state).get(curChar)) {
                        fromSListLast++;
